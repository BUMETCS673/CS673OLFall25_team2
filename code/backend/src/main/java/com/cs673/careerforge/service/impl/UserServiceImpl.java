...
public class UserServiceImpl implements UserService {
    ...
    public User createUser(User user) {
        ...
        if (existsByUsername(user.getUsername())) {
            throw new InvalidParamException("Username already exists: " + user.getUsername());
        }

        // Check if email already exists
        if (existsByEmail(user.getEmail())) {
            throw new InvalidParamException("Email already exists: " + user.getEmail());
        }
    ...
    public User updateUser(User user) {
        if (user.getId() == null) {
            throw new InvalidParamException("User ID is required for update");
        }

        User existingUser = userRepository.findById(user.getId())
                .orElseThrow(() -> new InvalidParamException("User not found with ID: " + user.getId()));
    ...
    public void deleteUser(Long id) {
        if (!userRepository.existsById(id)) {
            throw new InvalidParamException("User not found with ID: " + id);
        }
    ...
    public void deactivateUser(Long id) {
        User user = userRepository.findById(id)
                .orElseThrow(() -> new InvalidParamException("User not found with ID: " + id));
    ...
    public void changePassword(Long userId, String oldPassword, String newPassword) {
        ...
        if (newPassword == null || newPassword.trim().isEmpty()) {
            throw new InvalidParamException("New password is required");
        }

        if (newPassword.length() < 8) {
            throw new InvalidParamException("New password must be at least 8 characters long");
        }

        if (!newPassword.matches(".*[A-Za-z].*")) {
            throw new InvalidParamException("New password must contain at least one letter");
        }

        if (!newPassword.matches(".*\\d.*")) {
            throw new InvalidParamException("New password must contain at least one number");
        }
